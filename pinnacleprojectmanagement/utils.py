...
def update_multiple_field_options(field_updates):
    ...
    for update in field_updates:
        ...
        if not frappe.db.exists("DocField", {"parent": parent_doctype, "fieldname": fieldname_to_update}):
            frappe.logger().error(f"Field '{fieldname_to_update}' not found in '{parent_doctype}'.")
            continue
